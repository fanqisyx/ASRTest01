"""
TTS Service
- Subscribes to MQTT tts/speak and consumes SQLite tts tasks.
- Speaks via pyttsx3 queue manager, supports a simple stop command.
"""
import os
import sys
    ...
    def on_message(self, client, userdata, msg):
        try:
            ...
            if msg.topic == TOPIC_TTS_SPEAK:
                # enqueue DB task, let main loop claim
                if self.enable_sqlite:
...
def main():
    worker = TtsWorker()
    print('[TTS] service started. Press Ctrl+C to stop.')
    try:
        worker.loop()
